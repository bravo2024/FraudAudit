...
def test_data(): d=make_synthetic(500); assert d["X"].shape[0]==500 and 0.01<d["positive_rate"]<0.20
...
def test_cv(): d=make_synthetic(1000); cv=cross_validate(d,seed=42,n_folds=3); assert all(s["roc_auc"]["mean"]>0.5 for s in cv.values())
